...
async function startCommandInterface() {
    ...
    rl.on('line', async (input) => {
        ...
        if (command === 're') {
        ...
        } else if (command === 'autofill') { // ✅ 新增 autofill 命令
            ...
            for (const target of targets) {
                if (target.type() === 'page') {
                    ...
                    // 检查页面是否存在且是有效的 HTTP(s) 页面
                    if (page && page.url().startsWith('http')) {
                        currentPage = page;
                        // 如果有多个页面，可以考虑更复杂的逻辑来判断“聚焦”
                        // 例如，可以通过 page.evaluate(() => document.hasFocus()) 来尝试判断
                        // 但对于大多数用户场景，找到第一个符合条件的有效页面通常就够了
                        break;
                    }
                }
            }
            ...
            if (currentPage) {
                console.log(`[信息] 检测到当前聚焦页面: ${currentPage.url()}`);

                try {
                    await resumeAutofill.autofillResume(currentPage);
                    console.log('[指令] 自动填充完成。');
                } catch (err) {
                    console.error(`[错误] 自动填充页面 ${currentPage.url()} 时失败:`, err.message);
                    console.error(err); // 打印完整错误堆栈以便调试
                }
            } else {
                console.log('[提示] 未找到可自动填充的当前聚焦页面。请确保浏览器已打开简历填写页面。');
            }
        } else {
            console.log('[提示] 输入命令：re 或 edit 或 autofill')
        }
    })
}
